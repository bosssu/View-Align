...
namespace MacGruber
{
    public static partial class Utils
    {
		...
		{
			Transform t = atom.reParentObject.Find("object/rescaleObject/"+prefabName+"(Clone)");
			if (t == null)
				return null;
			else
				return t.gameObject;
        ...
        public static JSONStorableActionAudioClip SetupAudioAction(MVRScript script, string name, JSONStorableActionAudioClip.AudioClipActionCallback callback)
        {
            JSONStorableActionAudioClip action = new JSONStorableActionAudioClip(name, callback);
            script.RegisterAudioClipAction(action);
            return action;
        }
		...
		public static UIDynamicTextInfo SetupInfoTextNoScroll(MVRScript script, JSONStorableString storable, float height, bool rightSide)
		{
			...
			storable.setCallbackFunction += (string text) => { uid.text.text = text; };
			return uid;
		}
        ...
        public static void GetObjectScenePath(Transform target, StringBuilder stringBuilder)
        {
            stringBuilder.Insert(0, string.Format("/{0}", target.name));
            if (target.parent != null)
            {
                GetObjectScenePath(target.parent, stringBuilder);
            }
        }
		...
		public static void LogTransform(string message, Transform t, bool onlyTreeMap = true,bool includeComp = true)
		{
			StringBuilder b = new StringBuilder();
			b.Append(message).Append("\n");
			LogTransformInternal(t, 0, b, onlyTreeMap,includeComp);
			SuperController.LogMessage(b.ToString());
		}

		private static void LogTransformInternal(Transform t, int indent, StringBuilder b, bool onlyTreeMap, bool includeComp)
		{
			...
			if (t.childCount > 0)
			{
				b.Append(' ', indent*4+2).Append("|__:\n");
				for (int i=0; i<t.childCount; ++i)
				{
					Transform c = t.GetChild(i);
					LogTransformInternal(c, indent+1, b, onlyTreeMap,includeComp);
				}
			}
		}
	}
